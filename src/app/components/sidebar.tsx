    ...
    return (
        <>
            ...
            <aside className={`fixed top-0 left-0 w-72 bg-gray-100  h-screen p-5 z-40 transform transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
                ...
                <nav className='flex text-black justify-center'>
                    <ul>
                        {menuItems.map((item, index) => (
                            <li
                                key={index}
                                className={`mb-6 flex items-center cursor-pointer ${pathname === item.path ? 'text-[#5089FD]' : ''}`}
                ...
                <CloseIcon className="md:hidden text-4xl fixed top-5 right-5 z-50 bg-red-500 p-2 rounded"
